...
import DrawerTechnician from "@/components/drawerTechnician/drawerTechnician";
import SearchInput from "@/components/searchInput/searchInput";
import Spinner from "@/components/spinner/spinner";
import { SearchField } from "@/hook/useTechnician/types";
...
const ListTechnician = () => {
  ...
  const { data: searchedTechnicians } = useTechnicianSearch({
    searchTerm: searchParams,
    enabled: Boolean(debouncedSearch.trim()),
  });

  const displayedTechnicians = debouncedSearch.trim()
    ? searchedTechnicians ?? []
    : technicians;
      ...
      >
        ...
        >
          {rowVirtualizer.getVirtualItems().map((virtualRow) => {
            const technician = displayedTechnicians[virtualRow.index];
